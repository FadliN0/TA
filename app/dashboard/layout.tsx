...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const router = useRouter(); // <-- INSTANSIASI ROUTER
  ...
  useEffect(() => {
    const checkAuth = async () => {
      ...
      if (!session) {
        // Jika tidak ada tiket, buang ke halaman login
        router.replace('/login');
      } else {
        // Jika aman, ambil emailnya untuk ditampilkan di profil
  ...
  // TAMPILAN LOADING SEMENTARA (Agar dashboard tidak bocor)
  if (isChecking) {
    return (
  ...
  return (
    // Struktur utama layout: Tinggi layar penuh, overflow disembunyikan. 
    // KHUSUS PRINT: Tinggi menyesuaikan kertas, bg diubah putih.
        ...
        <div className="p-4 border-t border-slate-800 space-y-2">
          ...
          <div className="flex items-center gap-3 px-3 py-2 rounded-xl bg-slate-800/50">
            ...
            <div className="overflow-hidden">
              ...
              <p className="text-[10px] text-emerald-400 truncate font-bold">Administrator</p>
            </div>
          </div>
