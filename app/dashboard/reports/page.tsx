...
export default function AdvancedReportsPage() {
  ...
  // Tarik data klien sekali saja di tingkat induk agar bisa dipakai bersama oleh kedua tab
  useEffect(() => {
    supabase
      .from('customers')
        ...
        <div className="flex bg-slate-100 p-1.5 rounded-xl border border-slate-200 w-full lg:w-auto">
          <button 
            ...
            className={`flex-1 lg:flex-none px-4 py-2.5 rounded-lg text-sm font-black transition-all ${
              activeTab === 'transactions' ? 'bg-white shadow-sm text-blue-700' : 'text-slate-500 hover:text-slate-700'
            }`}
          >
          ...
          <button 
            ...
            className={`flex-1 lg:flex-none px-4 py-2.5 rounded-lg text-sm font-black transition-all ${
              activeTab === 'products' ? 'bg-white shadow-sm text-emerald-700' : 'text-slate-500 hover:text-slate-700'
            }`}
          >
            📦 Riwayat Barang
          </button>
