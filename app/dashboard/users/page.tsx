...
import { getUsers, updateUserPassword } from './tambah/actions';
    ...
    <div className="min-h-screen bg-stone-100">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 px-6 py-4 max-w-7xl mx-auto">
          ...
          <div className="flex items-center gap-3">
            ...
            <Link href="/dashboard/users/tambah">
              <button className="bg-[#0f3460] hover:bg-[#1a4b8a] text-white text-sm font-semibold
      ...
      <div className="p-6 max-w-7xl mx-auto space-y-5">
        ...
        <div className="grid grid-cols-3 gap-4">
          {[
            { label: 'Total Pengguna', value: users.length, icon: '👥', bg: 'bg-blue-50', text: 'text-blue-900', num: 'text-blue-800' },
            { label: 'Administrator',  value: roleCount.admin, icon: '🛡️', bg: 'bg-sky-50',  text: 'text-sky-900',  num: 'text-sky-800'  },
            { label: 'Staff / User',   value: roleCount.user,  icon: '👤', bg: 'bg-violet-50', text: 'text-violet-900', num: 'text-violet-800' },
          ].map((stat) => (
                    ...
                    filteredUsers.map((u, idx) => (
                      ...
                      >
                        ...
                        <td className="px-4 py-3.5">
                          <span className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded text-[11px] font-bold tracking-wide border
                            ${u.role === 'admin'
            ...
            <form onSubmit={handleUpdatePassword}>
              <div className="p-6 space-y-5">
                ...
                <div className="bg-slate-50 border border-slate-200 rounded-lg px-4 py-3 flex items-center gap-3">
                  <div className={`w-9 h-9 rounded-full flex items-center justify-center text-sm font-bold text-white flex-shrink-0 shadow-sm
                  ...
                  <div>
                    <div className="text-sm font-bold text-gray-800">{selectedUser.email}</div>
                    <div className="text-[11px] text-gray-400 mt-0.5">
                      {selectedUser.role.toUpperCase()} · {selectedUser.id.substring(0, 8)}…
                    </div>
                  </div>
                </div>
