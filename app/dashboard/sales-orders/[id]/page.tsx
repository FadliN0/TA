...
export default function SalesOrderDetailPage() {
  ...
  // State untuk melacak apakah DO atau Invoice SUDAH DIBUAT
  const [hasDO, setHasDO] = useState(false);
  const [hasInvoice, setHasInvoice] = useState(false);

  // State untuk tombol loading saat eksekusi
  ...
  const fetchSalesOrderData = async () => {
    ...
    try {
      ...
      // ── LOGIKA BARU: Cek eksistensi Surat Jalan (DO) ──
      const { count: doCount, error: doErr } = await supabase.from('delivery_orders').select('id', { count: 'exact', head: true }).eq('so_id', id);
      if (!doErr) setHasDO((doCount || 0) > 0);

      // ── LOGIKA BARU: Cek eksistensi Invoice ──
      const { count: invCount, error: invErr } = await supabase.from('invoices').select('id', { count: 'exact', head: true }).eq('so_id', id);
      if (!invErr) setHasInvoice((invCount || 0) > 0);
  ...
  if (loading) return (
    <div className="flex flex-col items-center justify-center min-h-[50vh] gap-4">
      ...
      <p className="text-slate-400 font-bold animate-pulse">Menarik data dokumen pesanan...</p>
    </div>
  );
  ...
  if (!salesOrder) return (
    <div className="card-modern max-w-xl mx-auto p-10 text-center mt-10">
      ...
      <p className="text-slate-500 mb-6">Dokumen Sales Order yang Anda cari tidak ada atau telah dihapus.</p>
      <Link href="/dashboard/sales-orders" className="btn-secondary inline-flex">Kembali ke Daftar SO</Link>
    </div>
  );
      ...
      <div className="print:hidden w-full max-w-[210mm] mx-auto card-modern p-5 md:p-6 border-l-4 border-l-slate-800 animate-in fade-in slide-in-from-top-4 duration-500 mb-6">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 w-full md:w-auto">
            <Link href="/dashboard/sales-orders" className="inline-flex items-center justify-center p-3 bg-slate-50 hover:bg-slate-100 text-slate-500 rounded-xl transition-colors border border-slate-200 shrink-0">
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 19l-7-7m0 0l7-7m-7 7h18" /></svg>
            </Link>
            
            <div className="w-full sm:w-auto">
              <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-1">Status SO</span>
              <select
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-end gap-3 w-full md:w-auto">
            <div className="flex items-center gap-2 flex-1 sm:flex-none">
              ...
              {/* Logika DO */}
              {salesOrder.status !== 'Cancelled' && salesOrder.status !== 'Completed' && !hasDO ? (
                <button
                ...
                >
                  ...
                  {isCreatingDO ? 'Memproses...' : 'Buat DO'}
                </button>
              ) : hasDO ? (
                 <Link href="/dashboard/delivery-orders" className="flex-1 sm:flex-none flex items-center justify-center gap-1.5 bg-emerald-50 hover:bg-emerald-100 text-emerald-700 border border-emerald-200 px-4 py-2 rounded-xl font-bold text-sm transition-colors h-[40px]">
                   ✓ Lihat DO
                 </Link>
              ) : null}

              {/* Logika Invoice */}
              {salesOrder.status !== 'Cancelled' && salesOrder.status !== 'Completed' && !hasInvoice ? (
                <button
                  onClick={handleCreateInvoice}
                  disabled={isCreatingInvoice}
                  className="flex-1 sm:flex-none flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-xl font-bold text-sm transition-colors shadow-blue-600/30 shadow-lg h-[40px]"
                >
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
                  {isCreatingInvoice ? 'Memproses...' : 'Buat INV'}
                </button>
              ) : hasInvoice ? (
                 <Link href="/dashboard/invoices" className="flex-1 sm:flex-none flex items-center justify-center gap-1.5 bg-blue-50 hover:bg-blue-100 text-blue-700 border border-blue-200 px-4 py-2 rounded-xl font-bold text-sm transition-colors h-[40px]">
                   ✓ Lihat INV
                 </Link>
              ) : null}
            </div>
            ...
            {/* Tombol Print */}
            <button onClick={handlePrint} className="w-full sm:w-auto flex items-center justify-center gap-2 bg-slate-800 hover:bg-slate-900 text-white px-4 py-2 rounded-xl font-bold text-sm transition-colors shadow-sm h-[40px] mt-2 sm:mt-0">
              🖨️ Cetak PDF
            </button>
          </div>
