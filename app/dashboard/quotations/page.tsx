...
export default function QuotationListPage() {
  ...
  const handleDelete = async (id: string, qNumber: string) => {
    const isConfirm = window.confirm(`Yakin ingin menghapus dokumen penawaran ${qNumber}?\nTindakan ini tidak bisa dibatalkan.`);
    if (!isConfirm) return;
  ...
  const StatusBadge = ({ status }: { status: string }) => {
    switch(status) {
      ...
      case 'Sent': 
        return <span className="bg-blue-100 text-blue-700 px-2.5 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider inline-block">Dikirim</span>;
      case 'Approved': 
  ...
  return (
    <div className="space-y-6 max-w-[1400px] mx-auto pb-10">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
        <div>
          <h1 className="text-2xl font-black text-slate-800">Dokumen Quotation</h1>
          <p className="text-sm text-slate-500">Kelola dan cetak surat penawaran harga alat berat.</p>
        </div>
        <Link href="/dashboard/quotations/create" className="btn-primary w-full md:w-auto">
      ...
      {loading ? (
        ...
        <div className="card-modern p-16 text-center flex flex-col items-center justify-center border-dashed border-2">
          ...
          <p className="text-sm text-slate-500 mt-1">Belum ada Quotation yang dibuat atau ditemukan.</p>
        </div>
      ) : (
        <>
          {/* ==========================================
          ...
          <div className="hidden md:block card-modern overflow-x-auto">
            <table className="w-full text-left border-collapse min-w-[900px]">
              <thead className="bg-slate-50 border-b border-slate-200">
                <tr>
                  <th className="p-4 text-xs font-bold text-slate-500 uppercase">No. Dokumen</th>
                  <th className="p-4 text-xs font-bold text-slate-500 uppercase">Tanggal Dibuat</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {filteredQuotations.map((q) => (
                  <tr key={q.id} className="hover:bg-slate-50 transition-colors group">
                    <td className="p-4">
                      <div className="font-mono text-sm font-bold text-blue-700">{q.quotation_number}</div>
                    </td>
                    <td className="p-4">
                      <div className="text-sm font-bold text-slate-800">{new Date(q.created_at).toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric' })}</div>
                      <div className="text-[10px] font-medium text-slate-400">Valid: {new Date(q.valid_until).toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric' })}</div>
                    </td>
                    <td className="p-4">
                      <div className="text-sm font-bold text-slate-800 uppercase">
                        {q.customers?.company_name || <span className="text-rose-400 italic">Klien Terhapus</span>}
                      </div>
                    </td>
          ...
          <div className="md:hidden space-y-4">
            {filteredQuotations.map((q) => (
              <div key={q.id} className="card-modern p-5 flex flex-col gap-3 relative">
                
                <div className="flex justify-between items-start border-b border-slate-100 pb-3">
                  <div>
                    <div className="font-mono text-sm font-bold text-blue-700">{q.quotation_number}</div>
                    <div className="text-xs text-slate-500 mt-0.5">{new Date(q.created_at).toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric' })}</div>
                  </div>
                  <StatusBadge status={q.status} />
                </div>
                ...
                <div>
                  ...
                  <div className="text-lg font-black text-slate-900">{fmtRp(q.grand_total || 0)}</div>
                </div>
